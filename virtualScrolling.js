function virtualScrolling(view, getLine, countOfAllLines, lineHeight) {
	...
	function render(scrollTop, rerenderingFlag) {
		...
		const 
			visibleRange = new Range(firstVisibleLineNum, lastVisibleLineNum + 1),
			oldCarriageRange = new Range(
		...
		if (firstVisibleLineNum != _.firstVisibleLineNum || rerenderingFlag) {
			...
			// console.log(`_.allLinesHeight`, _.allLinesHeight);
			// console.log(`countOfVisibleLines`, countOfVisibleLines);
			...
			if (oldR.first < newR.first) {
				...
				// console.log("newR.first < oldR.first");
			...
			if (oldR.afterLast < newR.afterLast) {
				...
				// console.log("oldR.afterLast < newR.afterLast");
				newR.arr.forEach((v) => {
					if (!oldR.includes(v)) 
				...
				// console.log("newR.afterLast < oldR.afterLast");
				while (
